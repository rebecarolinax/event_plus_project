...
const TipoUsuarioPage = () => {
  ...
  const [tipoUsuarios, setTipoUsuarios] = useState([]); //array
  const [notifyUser, setNotifyUser] = useState(); // componente notification
  const [showSpinner, setShowSpinner] = useState(false); // componente notification

  useEffect(() => {
    // define  a chamda da nossa api
    async function loadUsersType() {
      setShowSpinner(true);
      try {
        const retorno = await api.get(usersTypeResource);
        setTipoUsuarios(retorno.data);
        console.log(retorno);
      } catch (error) {
